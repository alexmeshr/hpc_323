...
int main(int argc, char* argv[]) {
  if (argv[1][0] == '+') {
    ...
    long_long start_virt, end_virt, start_real, end_real, values[4];
    ...
    int papi_params[] = {PAPI_L1_LDM, PAPI_L1_STM, PAPI_L1_DCM, PAPI_L2_DCM};
    for (int i = 0; i < 4; i++)
      if (PAPI_add_event(EventSet, papi_params[i]) != PAPI_OK)
        handle_error(i + 2);
